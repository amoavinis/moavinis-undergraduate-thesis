...
methods = ['vader', 'word2vec']
text_dims = [4, 300]

f = open(str('ensemble')+'_'+str(int(time.time()))+'.txt', 'w')
...
fc = Dense(True, True, lstm_dim, mode='ensemble', device=device).to(device)

print("Getting tweets into RAM...")
# Initialize Tweets data loader
tweets_loader1 = Load_twitter_data(NUM_TWEETS, method=methods[0], device=device)
tweets_loader2 = Load_twitter_data(NUM_TWEETS, method=methods[1], device=device)
print("OK.")

# Mean Square Error Loss Function
loss_fn = nn.CrossEntropyLoss()

# Calculating and printing the naive loss and accuracy for the validation dataset
tanh = nn.Tanh()
scores = [0, 0, 0]
for y in y_valid:
    scores[y.item()]+=1
...
for epoch in range(n_epochs):
    ...
    for x_batch, y_batch in train_loader:
        ...
        for i in range(x_batch.size(0)):
            tweets1 = tweets_loader1.get_embeddings(dates[pointer])
            tweets2 = tweets_loader2.get_embeddings(dates[pointer])
            pointer += 1
            if len(tweets1)==0:
                tweets1 = torch.zeros(text_dims[0]).to(device)
            if len(tweets2)==0:
                tweets2 = torch.zeros(text_dims[1]).to(device)
            text_outputs1.append(tweets1.tolist())
            text_outputs2.append(tweets2.tolist())
        text_outputs1 = torch.tensor(text_outputs1).float().to(device)
        text_outputs2 = torch.tensor(text_outputs2).float().to(device)
        
        # Through Financial LSTM model
        fin_lstm_out = stock_model(x_batch)

        # Pass LSTM and textual output into final layer
        yhat = fc(fin_lstm_out.to(device), text_outputs1.to(device), text_outputs2.to(device))
